...
async function getDownsampledBuffer(imagePath: string, maxDimension: number = 512): Promise<{ buffer: Buffer, info: sharp.OutputInfo }> {
...
export async function autoExposure(imagePath: string, currentWb?: WhiteBalanceOp): Promise<ExposureOp> {
  ...
  let ev = 0;
  if (median > 0.01) { // Avoid division by very small numbers
...
export async function autoContrast(
  ...
  let contrastAmt = 0;
  if (currentRange > 0.01) { // Avoid division by very small numbers
